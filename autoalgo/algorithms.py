...
def quick_select(nums: List[int], k: int) -> int:
    ...
    Optimized: Using numpy's partition for O(n) average case.
    """
    import numpy as np
    arr = np.array(nums)
    # np.partition finds the kth smallest in O(n) average case
    # It partially sorts so that arr[k] is the kth smallest
    # Elements before k are <= arr[k], elements after are >= arr[k]
